...
    # ----------------------------------------------------------------------
    # 🌟 CORRECCIÓN AQUÍ: Recibe la fecha_actualizacion como argumento para consistencia de TZ.
    def update_or_append_semaforo(self, id_casa: int, dias_atraso: int, saldo: float, estado: str, cuotas_pendientes: int, fecha_actualizacion: str) -> bool:
        """
...
        
        NOTA: El argumento fecha_actualizacion DEBE ser la hora local (GTM-5) ya formateada (AAAA-MM-DD HH:MM).
        """
...
                        "DIAS_ATRASO": int(float(row[2])) if row[2] and row[2].replace('.', '', 1).isdigit() else 0, # Columna C: DIAS_ATRASO
                        "ESTADO_SEMAFORO": row[3], # Columna D: ESTADO
                        "CUOTAS_PENDIENTES": int(float(row[4])) if row[4] and row[4].replace('.', '', 1).isdigit() else 0, # Columna E: CUOTAS
